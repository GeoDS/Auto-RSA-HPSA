...
from functions_walktrap import *
...
walktrap_folder = args.output + '/walktrap/'
if 'walktrap' not in os.listdir(args.output):
    os.mkdir(walktrap_folder)

subfolder = str(pop_u) + '_' + str(nct_u) + '_' + str(pop_r) + '_' + str(nct_r) + "_t2_" + str(t2) + "_perc_" + str(perc_limit)
output = walktrap_folder + subfolder
print(output)


if subfolder not in os.listdir(walktrap_folder):
    os.mkdir(output)
...
cluster_walk = g_un.community_walktrap(weights = "weight", steps = 5).as_clustering()
...
#output the result
result_df.to_csv("walktrap_cluster_result.csv", index = False)
