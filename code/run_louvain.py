...
louvain_folder = args.output + '/louvain/'
if 'louvain' not in os.listdir(args.output):
    os.mkdir(louvain_folder)

subfolder = str(pop_u) + '_' + str(nct_u) + '_' + str(pop_r) + '_' + str(nct_r) + "_t2_" + str(t2) + "_perc_" + str(perc_limit)
output = louvain_folder + subfolder
print(output)


if subfolder not in os.listdir(louvain_folder):
    os.mkdir(output)
...
cluster_walk = g_un.community_multilevel(weights = "weight")#.as_clustering()
...
#output the result
result_df.to_csv("louvain_cluster_result.csv", index = False)
